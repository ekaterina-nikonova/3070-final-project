...
The prototype utilises Perplexity's Sonar model via API.
...
The prototype uses a command-line interface (CLI) that requires learners to:
...
2. Automatic management of file encoding, format conversion, and cleanup can eliminate the need for user involvement in technical details.

3. Initially presented with a simple "Record Answer" button, advanced learners or educators could access settings for fine-tuning quality parameters.

### 4.3. Summary

The prototype successfully demonstrates the feasibility of an integrated system for vocabulary-constrained exercise generation and multimodal assessment. However, several avenues for improvement are evident:
